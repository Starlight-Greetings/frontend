...
export default function ReactionButtons({ message, onReact }) {
  ...
  return (
    <div className="flex justify-center items-center gap-3 pt-5 border-t border-white/15">
      {/* 1. 토닥토닥 (Heart) */}
      <button
        onClick={() => onReact(message.id, 'heart')}
        className={`flex-1 flex flex-col items-center justify-center py-2.5 px-2 rounded-2xl border transition-all duration-300 active:scale-95 ${
          userReacted.heart
      ...
      >
        ...
        <span className="text-[11px] font-semibold">토닥토닥</span>
        <span className="text-[10px] opacity-75 font-mono mt-0.5">{reacts.heart}</span>
      </button>
      ...
      >
        ...
        <span className="text-[11px] font-semibold">위로됐어요</span>
        <span className="text-[10px] opacity-75 font-mono mt-0.5">{reacts.cheer}</span>
      </button>
      ...
      >
        ...
        <span className="text-[11px] font-semibold">가라앉히기</span>
        <span className="text-[10px] opacity-60 font-mono mt-0.5">신고</span>
      </button>
    </div>
  );
}
